...
*/
ACCB1 void ACCB2 SignDocument (void* data) 
{ 
    AVAlertNote ("Signing document"); 
}

ACCB1 ASBool ACCB2 ComputeMarkedProc (void* data) 
{ 
    ASBool expressionorcondition = true; 
    if (expressionorcondition) return true; 
    else return false;
} 
ACCB1 ASBool ACCB2 ComputeEnabledProc (void* data) 
{ 
    if (AVAppGetNumDocs() > 0) return true; else return false; 
} 
...
ACCB1 void ACCB2 MyPluginCommand(void* clientData)
{
    ...
    AVComputeEnabledProc CompEnabledProcPtr = NULL; AVComputeMarkedProc CompMarkedProcPtr = NULL;
    ...
    ExecProcPtr = ASCallbackCreateProto (AVExecuteProc, &SignDocument); 
    CompEnabledProcPtr = ASCallbackCreateProto (AVComputeEnabledProc, &ComputeEnabledProc); 
    CompMarkedProcPtr = ASCallbackCreateProto (AVComputeMarkedProc, &ComputeMarkedProc); 
    //Retrieve the File menu 
    FileMenu = AVMenubarAcquireMenuByName(Themenubar, "File"); 
    if (FileMenu) 
    { 
        //Create a new menu item 
        NewItem = AVMenuItemNew ("Sign Document", "ADBE:ExternWin", NULL, true, NO_SHORTCUT, 0, NULL, gExtensionID); 
        if (NewItem == NULL) 
        { 
        ...
        AVMenuItemSetComputeEnabledProc (NewItem, CompEnabledProcPtr,NULL); 
        AVMenuItemSetComputeMarkedProc (NewItem, CompMarkedProcPtr,NULL); 
...
ACCB1 ASBool ACCB2 MyPluginSetmenu()
{
	...
	return PluginMenuItem("DocSign Plugin", "ADBE:DocSignPluginMenu");
}
